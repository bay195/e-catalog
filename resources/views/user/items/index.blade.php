...
<div class="py-5" style="background-color: #f0f2f5;"> 
    <div class="container">
        ...
        <div class="p-4 mb-5 bg-white shadow rounded border">
            <h2 class="mb-4 fw-bold fs-3">Daftar Item User</h2>

            <a href="{{ route('user.items.create') }}" class="btn btn-primary mb-3">Tambah Item</a>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($item->status == 0)
                            <a href="{{ route('user.items.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>

                            <form action="{{ route('user.items.destroy', $item->id) }}" method="POST" class="d-inline">
                                @csrf @method('DELETE')
                                <button onclick="return confirm('Yakin hapus?')" class="btn btn-sm btn-danger">Hapus</button>
                            </form>

                            <form action="{{ route('user.items.submit', $item->id) }}" method="POST" class="d-inline">
                                @csrf @method('PUT')
                                <button type="submit" class="btn btn-sm btn-primary">Submit ke FAT</button>
                            </form>
                        @else
                            <span class="text-muted">Sudah disubmit</span>
                        @endif
