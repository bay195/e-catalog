...
<div class="py-5" style="background-color: #f0f2f5;"> 
    <div class="container">
        ...
        <div class="p-4 mb-5 bg-white shadow rounded border">
            <h2 class="mb-4">Daftar Item Procurement</h2>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($item->status == 2)
                                <a href="{{ route('procurement.items.edit', $item->id) }}" class="btn btn-sm btn-warning">Edit</a>

                                <form action="{{ route('procurement.items.submit', $item->id) }}" method="POST" class="d-inline">
                                    @csrf @method('PUT')
                                    <button type="submit" class="btn btn-sm btn-primary">Submit ke Logistik</button>
                                </form>
                            @else
                                <span class="text-muted">Tidak ada aksi</span>
                            @endif
                            {{-- Tombol Hapus tetap ditampilkan untuk semua status --}}
                            <form action="{{ route('procurement.items.destroy', $item->id) }}" method="POST" class="d-inline">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="5" class="text-center">Tidak ada item yang perlu diproses oleh Procurement.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div> 
